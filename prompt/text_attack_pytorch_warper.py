...
class PyTorchModelWrapper(ModelWrapper):
    ...
    def __call__(self, text_input_list, batch_size=8):
        ic(batch_size)
        model_device = next(self.model.parameters()).device
        ids = self.tokenizer(text_input_list)

        for k in ids:
            ids[k] = torch.tensor(ids[k]).to(model_device)
        outputs = []
        with torch.no_grad():
            for i in range(0, len(ids['input_ids']),batch_size):
                outputs.append(self.model(**{'input_ids':ids['input_ids'][i:i+batch_size], 'attention_mask':ids['attention_mask'][i:i+batch_size]}))
        outputs = torch.cat(outputs).reshape([-1,5])
        ic(outputs.shape)
        return outputs
